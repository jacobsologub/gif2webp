...
//------------------------------------------------------------------------------
typedef struct GifBuffer_ {
    unsigned char* data;
    unsigned long length;
    unsigned long offset;
} GifBuffer;

int gifDataReadFunction (GifFileType* GifFile, GifByteType* buffer, int count) {
    GifBuffer* gif_buff = (GifBuffer*) GifFile->UserData;
    if (gif_buff->offset + count <= gif_buff->length) {
        memcpy (buffer, gif_buff->data + gif_buff->offset, count);
        gif_buff->offset += count;
    }
    else {
        fprintf (stderr, "! gif_buff->data_offset(%lu) + count(%d) <= gif_buff->data_len(%lu)\n", gif_buff->offset, count, gif_buff->length);
        return 0;
    }
    return count;
}

int convert (int argc, const char *argv[], char* inData, int inDataLength, WebPData* outputData) {
    int verbose = 0;
    ...
    for (c = 0; c < argc; ++c) {
        if (!strcmp(argv[c], "-lossy")) {
            ...
            config.method = (int) strtol(argv[++c], NULL, 0);
            ...
            config.filter_strength = (int) strtol(argv[++c], NULL, 0);
    ...
    GifBuffer gifBufferIn;
    gifBufferIn.length = inDataLength;
    gifBufferIn.offset = 0;
    gifBufferIn.data = (unsigned char*) inData;
    
    gif = DGifOpen (&gifBufferIn, gifDataReadFunction);
...
End:
    ...
    WebPDataCopy (&webp_data, outputData);
